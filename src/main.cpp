...
// Copyright (c) 2013  The jdcoin developer
...
// disabled temporarily, will be used in the future to fix minimum PoS difficulty at 0.25
...
const int64 nChainStartTime = 1383963638;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // jdcoin: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal jdcoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in jdcoin
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "In God We Trust -- Nov 8th 2013";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:jdcoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0xe0d697e253772d11e1e43d2e3ac17220b37da582dd50357ba9daf3b072169bf8"));
        block.print();
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        /* Don't allow old clients having ver < 0.1.4.3 */
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("jdcoinMiner : proof-of-work not meeting target");
...
void static ThreadjdcoinMiner(void* parg);

static bool fGeneratejdcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void jdcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGeneratejdcoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneratejdcoins) && !fProofOfStake)
                return;
        }
...
void static ThreadjdcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        jdcoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadjdcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadjdcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadjdcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generatejdcoins(bool fGenerate, CWallet* pwallet)
{
    fGeneratejdcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratejdcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d jdcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadjdcoinMiner, pwallet))
                printf("Error: NewThread(ThreadjdcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
